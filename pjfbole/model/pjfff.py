...
class PJFFF(GeneralRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.job_biLSTM = nn.LSTM(
            input_size=self.embedding_size * 2 + config['BERT_output_size'] * 2,
    ...
    def _get_fg_I_user(self, user_id, his_items):
        f_e = self._get_fg_E_user(user_id)
        his_g_e = self.bert_lr(self.bert_item[his_items])  # [2048, 100, 32]
        his_g_e = torch.cat([his_g_e, self.job_emb(his_items)], dim=2)
    ...
    def _get_fg_I_item(self, item_id, his_users):
        g_e = self._get_fg_E_item(item_id)
        his_f_e = self.bert_lr(self.bert_user[his_users])  # [2048, 100, 32]
        his_f_e = torch.cat([his_f_e, self.geek_emb(his_users)], dim=2)
    ...
    def _forward_I(self, user_id, item_id, his_items, his_users):
        f_i = self._get_fg_I_user(user_id, his_items)
        g_i = self._get_fg_I_item(item_id, his_users)
    ...
    def calculate_loss(self, interaction):
        ...
        score_I_neg = self._forward_I(user_id, neg_item_id, his_items, neg_his_users)
